...
// 加载历史记录（从 SQLite 数据库）— 保留作为公共 API
#[allow(dead_code)]
...
// 分页加载历史记录
pub fn load_history_page(offset: i64, limit: i64, status: Option<&str>) -> Result<Vec<HistoryItem>, String> {
    let conn = database::get_connection()
        .map_err(|e| format!("数据库连接失败: {}", e))?;
...
// 统计历史记录总数
pub fn count_history(status: Option<&str>) -> Result<i64, String> {
    let conn = database::get_connection()
        .map_err(|e| format!("数据库连接失败: {}", e))?;
...
fn build_history_query(
    ...
    match status {
        ...
        Some("failed") => {
            sql.push_str(" WHERE status IN ('failed', 'cancelled')");
        }
        _ => {}
    }
...
pub fn add_to_history(item: HistoryItem) -> Result<(), String> {
    ...
    conn.execute(
        "DELETE FROM download_history WHERE id NOT IN (
            SELECT id FROM download_history ORDER BY created_at DESC LIMIT 5000
        )",
        [],
    ).ok();
